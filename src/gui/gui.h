    ...
    while (!glfwWindowShouldClose(win))
    {
                     ...
                     NK_WINDOW_TITLE))
        {
            for (auto &row : listings->result)
            {
                ...
                std::stringstream price_label;
                price_label << row.listing.price.amount << " " << row.listing.price.currency;
                nk_label(ctx, price_label.str().c_str(), NK_TEXT_LEFT);

                nk_label(ctx, row.listing.account.name.c_str(), NK_TEXT_LEFT);
                nk_label(ctx, time_since(row.listing.indexed).c_str(), NK_TEXT_LEFT);
            }
        }
        nk_end(ctx);
